...
describe('features/orderDetail/components/DesktopOrdersTable', () => {
  const props = {
    orders: [
      {
        entity_id: 621636,
  ...
  test('it should be match snapshot', () => {
    const { asFragment } = render(<DesktopOrdersTable {...props} />);
    expect(asFragment(<DesktopOrdersTable {...props} />)).toMatchSnapshot();
  });
  ...
  test('it should be match snapshot when order empty', () => {
    ...
    const { asFragment } = render(<DesktopOrdersTable {...customProps} />);
    expect(asFragment(<DesktopOrdersTable {...props} />)).toMatchSnapshot();
  });
  ...
  test('it should be match snapshot when loading', () => {
    ...
    const { asFragment } = render(<DesktopOrdersTable {...customProps} />);
    expect(
      asFragment(<DesktopOrdersTable {...customProps} />),
    ).toMatchSnapshot();
  });
