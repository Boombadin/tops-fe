...
import './ShopByBrandList.scss';
import withLocales from '../../hoc/withLocales';
import withStoreConfig from '../../hoc/withStoreConfig';

@withLocales
@withStoreConfig
class ShopByBrandList extends PureComponent {
  initialSwiper(node) {
        ...
        >
          <div className="swiper-wrapper">
            {map(slides, (slide, idx) => (
              <ShopByBrandItem index={idx} slide={slide} key={uuidV4()} />
            ))}
          </div>
          <div className="swiper-scrollbar" />
        </div>
        <div className={`swiper-button-next swiper-button-next-brand`} />
        <div className={`swiper-button-prev swiper-button-prev-brand`} />
      </React.Fragment>
    );
  }
    ...
    return (
      <React.Fragment>
        {isSlider && isBanner && slideStore.length > 0 && (
          <div className="shop-by-brand-list-wrap">
            <HeadTitle
...
export default withBannerByName(ShopByBrandList, 'Shop in Shop');
